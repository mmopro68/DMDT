...
st.caption("Chiến lược tối ưu hóa nâng cao: Quét toàn bộ cổ phiếu HOSE, sử dụng bộ lọc xu hướng đa tầng và Rolling Sharpe nhằm chinh phục VN-Index.")
...
st.sidebar.markdown("---")
st.sidebar.subheader("📅 KHUNG THỜI GIAN HUẤN LUYỆN (IN-SAMPLE)")
...
st.sidebar.subheader("📅 KHUNG THỜI GIAN BACKTEST (OUT-OF-SAMPLE)")
...
if st.sidebar.button("🚀 KÍCH HOẠT HỆ THỐNG ALPHA MAX", type="primary"):
    if train_end >= test_start:
        ...
        with st.spinner("Hệ thống đang quét toàn bộ dữ liệu sàn HOSE và trích xuất Alpha..."):
            ...
            if df_train.empty:
                ...
                for ticker in stock_cols:
                    ...
                    # 1. BỘ LỌC XU HƯỚNG ĐA TẦNG NÂNG CAO (Golden Cross + Long-term Bull market)
                    ...
                    # 2. BỘ LỌC ĐỘNG LƯỢNG ĐIỀU CHỈNH RỦI RO (Rolling Sharpe ngắn hạn thay cho Động lượng thô)
                ...
                if df_raw.empty:
                    st.error("❌ Không tìm thấy cổ phiếu nào vượt qua bộ lọc xu hướng đại chu kỳ.")
                else:
                    # Chấm điểm dựa trên phân vị Percentile của Rolling Sharpe để tìm cổ phiếu tăng bền vững nhất
                    ...
                    # Thuật toán Sàng lọc đa ngành phòng vệ
                    portfolio_list = []
                    sector_counts = {}
                    ...
                    for idx, row in df_raw.iterrows():
                        ...
                        if count < max_stocks_per_sector:
                            portfolio_list.append({
                                ...
                                "Chỉ Số RSI": round(row["RSI"], 1), 
                                "Rolling Sharpe (1M)": round(row["Rolling_Sharpe"], 2), # ĐÃ CHỈNH SỬA TÊN HIỂN THỊ CHÍNH XÁC
                                "Điểm Đánh Giá Tín Hiệu": round(row["Total_Score"], 3),
                                "Biến Động Tập HL": row["Vol_Raw"]
                            })
                    ...
                    if not portfolio_list:
                        st.error("❌ Không có mã cổ phiếu nào vượt qua cấu trúc bộ lọc xu hướng an toàn.")
                    else:
                    ...
                    # HIỂN THỊ: ĐÁNH GIÁ HIỆU QUẢ DANH MỤC
                    st.markdown('<div class="report-card">', unsafe_allow_html=True)
                    st.subheader("📊 Đánh giá hiệu quả danh mục")
                    
                    scan_test_tickers = list(set(sorted_tickers + ALL_HOSE_TICKERS))
                    df_test = load_all_market_data(scan_test_tickers, test_start, test_end)
                    
                    if df_test.empty or 'VNINDEX' not in df_test.columns:
                        st.error("❌ Không trích xuất được dữ liệu kiểm thử thực tế của giai đoạn backtest.")
                    else:
                        ...
                        df_metrics_final = pd.DataFrame({
                            ...
                            "DANH MỤC ALPHA MAX NÂNG CẤP": [f"{p_ann*100:.2f}%", f"{p_sd*100:.2f}%", f"{p_sharpe:.4f}", f"{p_max_dd*100:.2f}%"],
                        ...
                        ax.plot(cum_portfolio.index, cum_portfolio.values, color='#1ebd9d', lw=2.8, label="Chiến lược Alpha Max Tinh Chỉnh (Uptrend dài hạn)")
                        ax.plot(cum_base.index, cum_base.values, color='#ff9f43', lw=1.5, linestyle='--', label="Chiến lược Cơ sở (Mua & Nắm giữ toàn sàn)")
    ...
    st.info("💡 Hệ thống đã được tái cấu trúc sang cấu trúc mô hình tối ưu hóa toàn sàn HOSE. Hãy thiết lập khung tham số và nhấn nút khởi chạy chiến lược.")
