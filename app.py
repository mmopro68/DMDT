...
# ==========================================
# 4. HÀM TẢI DỮ LIỆU TỪ VNSTOCK (BAO GỒM VNINDEX CHỐNG LỖI)
# ==========================================
...
def load_stock_and_market_data(ticker_list, start, end):
    ...
    for i, ticker in enumerate(ticker_list):
        try:
            ...
            continue
        progress_bar.progress((i + 1) / (len(ticker_list) + 1), text=f"Đang phân tích chuỗi dữ liệu mã: {ticker}")
    
    # Tải dữ liệu chỉ số VNINDEX làm Benchmark thị trường
    try:
        ...
        if df_vnindex is not None and not df_vnindex.empty:
            ...
            if date_col:
                df_vnindex[date_col] = pd.to_datetime(df_vnindex[date_col]).dt.strftime('%Y-%m-%d')
                df_vnindex = df_vnindex.drop_duplicates(subset=[date_col])
                df_vnindex = df_vnindex.set_index(date_col)
                close_prices['VNINDEX'] = pd.to_numeric(df_vnindex['close'], errors='coerce')
    except:
        pass
...
# Hàm tính toán Maximum Drawdown
...
if st.sidebar.button("🚀 Kích Hoạt Bộ Lọc Phân Lớp Toàn Thị Trường", type="primary"):
    with st.spinner("Đang xử lý ma trận chỉ báo toán học so sánh đối chiếu toàn diện..."):
        ...
        if df_prices.empty or 'VNINDEX' not in df_prices.columns:
            st.error("❌ Không lấy được dữ liệu chỉ số VNINDEX hoặc rổ cổ phiếu để so sánh.")
        else:
            ...
            if strategy_option == "Phân bổ đều (Equal Weight)":
                ...
                if num_assets >= 2:
                    ...
                    for j in range(2, num_assets): weights[j] = remaining_weight
                else:
                    weights[0] = 1.0

            # --- BƯỚC 4: TÍNH TOÁN HIỆU SUẤT ĐỐI CHIẾU SONG PHƯƠNG ---
            df_all_returns = df_prices.pct_change().dropna()
            
            # 1. Tỷ suất sinh lời Danh mục Chiến lược (Hạt giống đa ngành)
            portfolio_returns = df_all_returns[sorted_tickers].dot(weights)
            cum_portfolio = (1 + portfolio_returns).cumprod()
            
            # 2. Tỷ suất sinh lời Thị trường chung (VNINDEX)
            vnindex_returns = df_all_returns['VNINDEX']
            cum_vnindex = (1 + vnindex_returns).cumprod()
            
            # 3. Tỷ suất sinh lời Chiến lược Cơ sở (Mua và nắm giữ đều tay TẤT CẢ các mã có trong rổ quét ban đầu)
            ...
            # --- BƯỚC 5: TÍNH CÁC CHỈ TIÊU ĐỊNH LƯỢNG NÂNG CAO ---
            # Annualized Returns
            p_mean = portfolio_returns.mean() * trading_days
            m_mean = vnindex_returns.mean() * trading_days
            b_mean = base_returns.mean() * trading_days
            
            # Annualized Volatilities
            p_std = portfolio_returns.std() * np.sqrt(trading_days)
            m_std = vnindex_returns.std() * np.sqrt(trading_days)
            b_std = base_returns.std() * np.sqrt(trading_days)
            ...
            # Thiết lập bảng so sánh 3 thực thể
            ...
            with col_m2:
                ...
                st.metric("Hệ số Alpha Jensen ($\alpha$ thặng dư)", f"{alpha_jensen*100:.2f}%", help="Lợi nhuận thặng dư kiếm được vượt ngoài kỳ vọng rủi ro. Càng lớn dương càng chứng minh thuật toán thông minh.")
            
            # Biểu đồ so sánh 3 đường cong tài sản
            ...
            ax.plot(cum_base.index, cum_base.values, color='orange', lw=1.8, linestyle='--', label="Chiến Lược Cơ Sở (Buy & Hold Rổ Quét)")
    ...
    st.info("💡 Hệ thống đã được tích hợp đầy đủ hệ thống Benchmark cao cấp (VN-Index, Buy&Hold, Alpha Jensen, Beta, Max Drawdown). Hãy nhấn nút để khởi chạy.")
