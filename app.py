...
st.caption("Thuật toán tối ưu hóa danh mục: Huấn luyện chọn lọc hạt giống trong quá khứ (In-Sample) và Kiểm thử thực tế độc lập (Out-of-Sample).")
...
# SỬA ĐỔI QUAN TRỌNG: TÁCH BIỆT RÕ RÀNG HAI KHUNG THỜI GIAN ĐẦU VÀO
...
# ==========================================
# 4. HÀM TẢI DỮ LIỆU TỪ VNSTOCK ĐỒNG BỘ CẢ HAI GIAI ĐOẠN
# ==========================================
...
if st.sidebar.button("🚀 Kích Hoạt Mô Hình Tối Ưu Tách Biệt", type="primary"):
    if train_end >= test_start:
        ...
        with st.spinner("Đang thực thi huấn luyện trên tập dữ liệu In-Sample..."):
            # 1. Tải dữ liệu tập HUẤN LUYỆN (In-Sample)
            ...
            if df_train.empty:
                ...
                # --- BƯỚC 1: CHẤM ĐIỂM TÍN HIỆU TẠI NGÀY CUỐI CÙNG CỦA TẬP HUẤN LUYỆN ---
                all_signal_data = {}
                ...
                for ticker in stock_cols:
                    ...
                    current_price = series.iloc[-1] # Giá tại ngày cuối tập huấn luyện
                ...
                # --- BƯỚC 2: PHÂN NGÀNH VÀ CHỌN RA DANH MỤC HẠT GIỐNG TẠI THỜI ĐIỂM CHỐT HUẤN LUYỆN ---
                st.subheader(f"🎯 Kết Quả Sàng Lọc Hạt Giống Tại Ngày Cuối Tập Huấn Luyện ({train_end.strftime('%d/%m/%Y')})")
                        ...
                        if valid_tickers:
                            ...
                            if not df_sector.empty:
                                ...
                                portfolio_details.append({
                                    ...
                                    "Giá Chốt Tập HL": df_sector.iloc[0]["Giá Chốt HL"],
                                    "Chỉ Số RSI Tập HL": df_sector.iloc[0]["RSI_HL"],
                ...
                # --- BƯỚC 3: TÍNH TỶ TRỌNG VỐN CỐ ĐỊNH CHO DANH MỤC ĐẦU TƯ ---
                ...
                # --- BƯỚC 4: MANG DANH MỤC ĐI KIỂM THỬ TRÊN GIAI ĐOẠN MỚI (Out-of-Sample Backtest) ---
                with st.spinner("Đang tiến hành kiểm thử độc lập Out-of-Sample..."):
                    ...
                    if df_test.empty or 'VNINDEX' not in df_test.columns:
                        ...
                        # --- BƯỚC 5: TÍNH CÁC CHỈ TIÊU ĐỊNH LƯỢNG THỰC TẾ TRÊN TẬP KIỂM THỬ ---
                        ...
                        # Hệ số Beta & Alpha Jensen thực tế trên tập kiểm thử độc lập
                        ...
                        df_compare_metrics = pd.DataFrame({
                            ...
                            "Danh Mục Chiến Lược (Thuật toán chọn trước)": [f"{p_mean*100:.2f}%", f"{p_std*100:.2f}%", f"{p_sharpe:.4f}", f"{p_max_dd*100:.2f}%"],
                        ...
                        ax.plot(cum_portfolio.index, cum_portfolio.values, color='teal', lw=2.5, label=f"Danh Mục Chiến Lược Thực Tế ({strategy_option})")
    ...
    st.info("💡 Mô hình đã được nâng cấp lên chuẩn tách biệt cấu trúc dữ liệu khoa học In-Sample và Out-of-Sample. Hãy cài đặt khung thời gian ở Sidebar và nhấn nút Khởi chạy.")
